...
public class MCurso {
	...
	public List<Curso> searchCursos(String arg) {
		...
		try {
			...
			if (list == null || list.isEmpty()) {
				errorMsg = "Não Foi encontrado nenhum Curso similar a '" + arg
	...
	public List<Curso> popularesCursos() {
		...
		try {
			...
			if (list == null || list.isEmpty()) {
				errorMsg = "Não foi encontrado nenhum curso";
				return null;
			}
			// fechando a conexao
		} catch (HibernateException e) {
	...
	public List<Curso> cursosGratis() {
		...
		try {
			...
			String sql = "FROM Curso c WHERE price = 0";
			...
			if (list == null || list.isEmpty()) {
				errorMsg = "Não foi encontrado nenhum curso gratuito";
				return null;
			}
			// fechando a conexao
		} catch (HibernateException e) {
	...
	public List<Curso> ultimosCursos() {
		...
		try {
			...
			String sql = "FROM Curso c ORDER BY id DESC";
